...
module.exports = (context, callback) => {
    ...
    if (dom.window.document.querySelector(`[${sml_pagination_src}]`)) {
        ...
        for (var page = 1; page <= page_max; page++) {
            ...
            context.data.pagination.page_max = page_max;
            ...
            context.data.path = context.path_target.replace(/\\/g,"/").replace(/^.*?\//,'');
            context.data.basename = context.path_target.replace(/\\/g,"/").replace(/^.*\//,'');
            context.data.pagination.data = data_source[keys[context.data.pagination.offset]];
            callback();
        }
    } else {
        context.data.path = context.path_target.replace(/\\/g,"/").replace(/^.*?\//,'');
