module.exports = class Proto extends (require('./smloabs')) {
    conv() {
        ...
        const sml_loop_min = `${this.context.settings.sml_prefix}-loop-min`;
        this.context.dom.window.document.querySelectorAll(`[${sml_loop}]`).forEach((d) => {
            ...
            const loop_min = parseInt(d.getAttribute(sml_loop_min));
            ...
            for (var i of Object.keys(data_source)) {
                cnt++;
                if(cnt >= loop_max + 1) break;
                if(cnt < loop_min) continue;
                if (this.context.data.pagination && this.context.data.pagination.src_name == src_name) {
